(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Saudi Arabia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Saudi Arabia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-sa-entity?
                                       true` (near-universal for Saudi Arabia
                                       ...
                                       Grounded in Saudi commercial
                                       registration typically required for
                                       Etimad participation (contracting
                                       authorities / LTP).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CR/統一資格/Etimad登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- sa-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-sa-entity? true`, INDEPENDENTLY verify
  `:has-sa-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-sa-entity?` ground truth (most Saudi Arabia public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-sa-entity? e))
                 (not (true? (:has-sa-entity? e))))
        [{:rule :sa-entity-missing
          :detail (str subject " はサウジアラビア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (sa-entity-missing-violations request st)
