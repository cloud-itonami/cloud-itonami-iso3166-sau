# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Saudi Arabia

## Classification

- Repository: `cloud-itonami-iso3166-sau`
- ISO 3166: `SAU` (Saudi Arabia)
...
- Social impact: [:local-content-scoring-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Saudi
  public contract
...
- registration walkthrough for Etimad, the Saudi government's unified e-procurement and e-payment platform for government contracts
- business/tax registration checklist: a Commercial Registration (CR) from the Ministry of Commerce, plus Chamber of Commerce membership where required; VAT registration with ZATCA (Zakat, Tax and Customs Authority) where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: Local Content and Government Procurement Authority (LCGPA) local-content scoring, and Saudization (Nitaqat) workforce-localization requirements on qualifying contracts
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Saudi-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Saudi Arabia).
